...
public class NiuchanChanPerformer extends ChanPerformer
{
	...
	{
		NiuchanChanLocator locator = ChanLocator.get(this);
		if (data.isCatalog())
		{
			...
			try
			{
				posts = new NiuchanPostsParser(responseText, this, data.boardName).convertPosts();
		...
		else
		{
			...
			try
			{
				return new ReadThreadsResult(new NiuchanPostsParser(responseText, this, data.boardName)
						.convertThreads());
			}
			catch (ParseException e)
			{
			...
			InvalidResponseException
	{
		...
		try
		{
			return new ReadPostsResult(new NiuchanPostsParser(responseText, this, data.boardName).convertPosts());
		}
		catch (ParseException e)
		{
			throw new InvalidResponseException(e);
	...
	public ReadSinglePostResult onReadSinglePost(ReadSinglePostData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			Post post = new NiuchanPostsParser(responseText, this, data.boardName).convertSinglePost();
	...
	public ReadBoardsResult onReadBoards(ReadBoardsData data) throws HttpException, InvalidResponseException
	{
		...
		try
		{
			return new ReadBoardsResult(new NiuchanBoardsParser(responseText).convert());
		}
		catch (ParseException e)
		{
			throw new InvalidResponseException(e);
	...
	public SendPostResult onSendPost(SendPostData data) throws HttpException, ApiException, InvalidResponseException
	{
		...
		entity.add("em", data.optionSage ? "sabbia" : data.email);
		...
		Uri.Builder builder = new Uri.Builder().scheme("https").authority("www.niuchan.org");
		...
		try
		{
			...
			new HttpRequest(uri, data.holder, data).addHeader("Referer", "https://www.niuchan.org").setPostMethod(entity)
					.setRedirectHandler(HttpRequest.RedirectHandler.NONE).execute();
		...
		if (matcher.find())
		{
			...
			CommonUtils.writeLog("Niuchan send message", errorMessage);
			...
			InvalidResponseException
	{
		...
		if (responseText != null)
		{
			...
			CommonUtils.writeLog("Niuchan delete message", responseText);
		}
		throw new InvalidResponseException();
	}
			...
			InvalidResponseException
	{
		...
		if (responseText != null)
		{
			...
			CommonUtils.writeLog("Niuchan report message", responseText);
		}
		throw new InvalidResponseException();
	}
}